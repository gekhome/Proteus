...
{
    public class TableDeltioFoitisiService : ITableDeltioFoitisiService, IDisposable
    {
        private readonly ProteusDBEntities entities;

        public TableDeltioFoitisiService(ProteusDBEntities entities)
        {
        ...
        public string Create(int studentId, int schoolId)
        {
            ...
            var egrafes_data = (from d in entities.ΜΑΘΗΤΕΣ_ΕΓΓΡΑΦΕΣ
                                where d.ΜΑΘΗΤΗΣ_ΚΩΔ == studentId && (d.ΦΟΙΤΗΣΗ == 1 || d.ΦΟΙΤΗΣΗ == 2 || d.ΦΟΙΤΗΣΗ == 6 || d.ΦΟΙΤΗΣΗ == 7 || d.ΦΟΙΤΗΣΗ == 3)
            ...
            {
                var target_data = (from d in entities.FOITISI_DELTIA where d.ΜΑΘΗΤΗΣ_ΚΩΔ == item.ΜΑΘΗΤΗΣ_ΚΩΔ && d.ΚΩΔ_ΤΜΗΜΑ == item.ΚΩΔ_ΤΜΗΜΑ select d).ToList();

                if (target_data.Count == 0)
                    result = CreateFDKRecordSet(item.ΜΑΘΗΤΗΣ_ΚΩΔ, item.ΚΩΔ_ΤΜΗΜΑ, item.ΕΓΓΡΑΦΗ_ΕΙΔΟΣ, item.ΦΟΙΤΗΣΗ, schoolId);
                if (result != 0)
                {
        ...
        {
            string msg = "Η ενημέρωση του πίνακα δελτίου φοίτησης του σπουδαστή ολοκληρώθηκε.";

            // STEP 1: Remove data for this dtudent
            var target_data = (from d in entities.FOITISI_DELTIA where d.ΜΑΘΗΤΗΣ_ΚΩΔ == studentId select d).ToList();
            if (target_data.Count > 0)
            {
                foreach (var item in target_data)
                {
                    if (item != null)
                    {
                        ...
                        entities.FOITISI_DELTIA.Remove(item);
        ...
        {
            string msg = "Η διαγραφή του πίνακα δελτίου φοίτησης του σπουδαστή ολοκληρώθηκε.";

            var target_data = (from d in entities.FOITISI_DELTIA where d.ΜΑΘΗΤΗΣ_ΚΩΔ == studentId select d).ToList();
            if (target_data.Count == 0)
            {
                msg = "Δεν βρέθηκαν στοιχεία δελτίου φοίτησης για τον επιλεγμένο σπουδαστή. Η διαγραφή ακυρώθηκε.";
                return msg;
            }
            foreach (var item in target_data)
            {
        ...
        private int CreateFDKRecordSet(int? studentId, int? tmimaId, int? egrafi_type, int? foitisi, int iek)
        {
            ...
            {
                FOITISI_DELTIA entity = new FOITISI_DELTIA()
                {
                ...
                entities.FOITISI_DELTIA.Add(entity);
                entities.SaveChanges();
            }
            return 0;
        }
